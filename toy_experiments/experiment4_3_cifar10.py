...
handler = logging.FileHandler('exp4_3_cifar10.log', 'w', 'utf-8') # or whatever
...
class HyperNetDynamic(nn.Module):
    ...
    def forward(self, context=None, recursive_input=False):
        ...
        x2 = self.convt2(x1)
        x2 = F.relu(x2)
        x3 = self.convt3(x2+x1)
        x3 = F.relu(x3)
        x4 = self.convt4(x3+x2)
        x4 = F.relu(x4)
        x5 = self.convt5(x4+x3)
        x5 = F.relu(x5)
        self.last_output = x5
        return x5
...
class HyperNetLinearDynamicHead(nn.Module):
    def __init__(self, in_features, out_features,
        hyperbase,
        recursion_scheduler=None,
        max_recursions=10):
        super(HyperNetLinearDynamicHead, self).__init__()
        self.hyperbase = hyperbase
...
class Net(nn.Module):
    def __init__(self, num_channels, num_classes):
        ...
        self.hl1 = HyperNetLinearDynamicHead(32*32*num_channels, self.num_classes,
...
def train(args, model, device, train_loader, optimizer, epoch, criterion):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        start = timer()
        output = model(data, recursion_criteria=entropy_criteria)
        end = timer()
        current_epoch = epoch
...
class ResNet50HyperElf(nn.Module):
    ...
    def forward(self, x, recursion_criteria=None):
        ...
        x = self.layer1(x)
        y1 = self.hl1(x, recursion_criteria=recursion_criteria)
        # Exit here if recursive_criteria is true
        if recursion_criteria is not None:
            if not recursion_criteria(y1):
                return y1

        x = self.layer2(x)
        y2 = self.hl2(x, recursion_criteria=recursion_criteria)
        # Exit here if recursive_criteria is true
        if recursion_criteria is not None:
            if not recursion_criteria(y2):
                return y2

        x = self.layer3(x)
        y3 = self.hl3(x, recursion_criteria=recursion_criteria)
        # Exit here if recursive_criteria is true
        if recursion_criteria is not None:
            if not recursion_criteria(y3):
                return y3
...
def main():
    ...
    # More on NLL vs CE: https://towardsdatascience.com/cross-entropy-negative-log-likelihood-and-all-that-jazz-47a95bd2e81
    criterion = nn.CrossEntropyLoss() # ResNet
    #criterion = F.nll_loss # Simple ad-hoc Net

    scheduler = StepLR(optimizer, step_size=1, gamma=args.gamma)
    for epoch in range(1, args.epochs + 1):
        train(args, model, device, train_loader, optimizer, epoch, criterion=criterion)
        test(model, device, test_loader, criterion=criterion)
        scheduler.step()

    if args.save_model:
        torch.save(model.state_dict(), "cifar10_43cnn.pt")
