...
handler = logging.FileHandler('exp5_3_cifar10.log', 'w', 'utf-8') # or whatever
...
class Net(nn.Module):
    def __init__(self, num_channels, num_classes):
        ...
        self.hl1 = HyperLinear(32*32*num_channels, self.num_classes,
            self.hb, iteration_scheduler=self.iteration_scheduler) # HyperLiner1

    def forward(self, x, early_exit_criteria=None):
        x = self.hl1(x, early_exit_criteria=early_exit_criteria)
...
def main():
    ...
    if args.save_model:
        torch.save(model.state_dict(), "cifar10_53cnn.pt")
