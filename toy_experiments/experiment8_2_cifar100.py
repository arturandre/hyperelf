...
- The Classification Network has just HyperLinear layers.
...
handler = logging.FileHandler('exp8_2_cifar100.log', 'w', 'utf-8') # or whatever
...
class Net(nn.Module):
    def __init__(self, base_model, num_classes) -> None:
        ...
        self.hfc1 = None
        self.hfc2 = HyperLinear(self.hyperbase_extractor, 512, 512)
        self.hfc3 = HyperLinear(self.hyperbase_extractor, 512, 64)
        self.hfc4 = HyperLinear(self.hyperbase_extractor, 64, num_classes)
        
        
    def forward(self, x):
        if self.hfc1 is None:
            image_numel = 1
            for i in x.shape[1:]:
                image_numel *= i
            self.hfc1 = HyperLinear(self.hyperbase_extractor, image_numel, 512).to(x.device)
        f = self.hfc1(x, x) #image, features
        f = F.relu(f)
        #x = torch.flatten(x, 1)
        f = self.hfc2(x, f) #image, features
        f = F.relu(f)
        f = self.hfc3(x, f) #image, features
        f = F.relu(f)
        f = self.hfc4(x, f) #image, features
        f = F.relu(f)
...
def main():
    ...
    if args.save_model:
        torch.save(model.state_dict(), "cifar100_82cnn.pt")
