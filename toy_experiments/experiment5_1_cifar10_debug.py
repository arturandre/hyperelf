...
handler = logging.FileHandler('exp5_1_cifar10debug.log', 'w', 'utf-8') # or whatever
...
class HyperNetDynamicHead(nn.Module):
    ...
    def reiterate(self, recursion_criteria=None):
...
class HyperLinear(nn.Module):
    def __init__(self, in_features, out_features,
        ...
        params = {
            "hyperbase": hyperbase,
            "recursion_scheduler": recursion_scheduler,
    ...
    def forward(self, x, recursion_criteria=None):
        global current_epoch
        weights = self.hyperhead_w(x, recursion_criteria=recursion_criteria)
        weights = weights.view(-1, self.in_features, self.out_features)
        bias = self.hyperhead_b(x, recursion_criteria=recursion_criteria)
        input_shape = x.shape
        x = torch.flatten(x, 1)
        ...
        if recursion_criteria is not None:
            ...
            while recursion_criteria(y):
                ...
                if (verbose > 0):
                    ...
                    save_image(x.view(input_shape), f"image{num_recursions}.png")
...
def main():
    ...
    if args.save_model:
        torch.save(model.state_dict(), "cifar10_51debugcnn.pt")
