...
- HyperConv2D is used to compute the parameters
  just for the first conv layer in resnet. (Resnet50+Hyper)
- Early exits are removed. (NoExyl)
...
nlogger = None
project_tags = ["Resnet50+Hyper", "NoExyl", "ResolutionNormal"]

root_logger= logging.getLogger()
root_logger.setLevel(logging.DEBUG) # or whatever
handler = logging.FileHandler('exp7_2_1_cifar100.log', 'w', 'utf-8') # or whatever
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            start = timer()
            #output, intermediate_outputs = model(data, EntropyCriterion(), test=True)
...
def main():
    ...
    model = ResNet50Hyper(resnet_base, num_channels=num_channels, num_classes=num_classes)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if correct_test > correct_test_max:
            ...
            if args.save_model:
                torch.save(model.state_dict(), "cifar100_721cnn.pt")
        scheduler.step()
