...
handler = logging.FileHandler('exp8_1_cifar100.log', 'w', 'utf-8') # or whatever
...
class Net(nn.Module):
    def __init__(self, base_model, num_classes) -> None:
        ...
        self.hfc1 = None
        self.hfc2 = HyperLinear(self.hyperbase_extractor, 64,num_classes)
    ...
    def forward(self, x):
        if self.hfc1 is None:
            ...
            for i in x.shape[1:]:
                image_numel *= i
            self.hfc1 = HyperLinear(self.hyperbase_extractor, image_numel, 64).to(x.device)
...
def main():
    ...
    if args.save_model:
        torch.save(model.state_dict(), "cifar100_81cnn.pt")
