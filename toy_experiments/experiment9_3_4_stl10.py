"""
* This experiment is just an updated version of 9-3 such that
  now the best test acc network AND the last one are saved.
- Here we train on the STL10 dataset to use the trained
...
handler = logging.FileHandler('exp9_3_4_stl10.log', 'w', 'utf-8') # or whatever
...
def main():
    ...
    parser.add_argument('--hold-exit', action='store_true', default=False,
                        help='Keeps an instance until the last exit regardless of exit criteria.')
    parser.add_argument('--log-it-folder', type=str,
                        help='For saving intermediate exited logs and images.')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if correct_test > correct_test_max:
            ...
            if args.save_model:
                torch.save(model.state_dict(), "stl10_934cnn.pt")
        if args.save_model:
            torch.save(model.state_dict(), "stl10_934cnn-last.pt")
        scheduler.step()
