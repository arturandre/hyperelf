"""
- resnet50 baseline again (Baseline)
- ! The ResNet is not pre-trained now (NoPretrain)
- original resolution (ResolutionNormal)
"""
...
from networks.base_resnet50 import ResNet50
...
nlogger = None
project_tags = ["Baseline", "NoPretrain", "ResolutionNormal"]

root_logger= logging.getLogger()
root_logger.setLevel(logging.DEBUG) # or whatever
handler = logging.FileHandler('exp7_5_1_cifar100.log', 'w', 'utf-8') # or whatever
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            start = timer()
            #output = model(data, EntropyCriterion(), test=True)
            output = model(data, EntropyCriterion())
            end = timer()
            intermediate_outputs = []
...
def main():
    ...
    model = ResNet50(resnet_base, num_channels=num_channels, num_classes=num_classes)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if correct_test > correct_test_max:
            ...
            if args.save_model:
                torch.save(model.state_dict(), "cifar100_751cnn.pt")
        scheduler.step()
