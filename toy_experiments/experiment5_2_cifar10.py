...
handler = logging.FileHandler('exp5_2_cifar10.log', 'w', 'utf-8') # or whatever
...
class HyperNetDynamicHead(nn.Module):
    ...
    def reiterate(self, early_exit_criteria=None):
...
class HyperLinear(nn.Module):
    def __init__(self, in_features, out_features,
        ...
        self.hyperhead_w = HyperNetDynamicHead(
            in_features,
            in_features*out_features,
            **params)
        self.hyperhead_b = HyperNetDynamicHead(
            in_features,
            out_features,
            **params)
        self.in_features = in_features
    ...
    def forward(self, x, early_exit_criteria=None):
        global current_epoch
        weights = self.hyperhead_w(x, early_exit_criteria=early_exit_criteria)
        weights = weights.view(-1, self.in_features, self.out_features)
        bias = self.hyperhead_b(x, early_exit_criteria=early_exit_criteria)
        input_shape = x.shape
        x = torch.flatten(x, 1)
...
class Net(nn.Module):
    def __init__(self, num_channels, num_classes):
        ...
        self.hl1 = HyperLinear(32*32*num_channels, self.num_classes,
            self.hb, iteration_scheduler=self.rs) # HyperLiner1

    def forward(self, x, gt=None):
        x = self.hl1(x, early_exit_criteria=EntropyCriterion(gt=gt))
...
def main():
    ...
    if args.save_model:
        torch.save(model.state_dict(), "cifar10_52cnn.pt")
