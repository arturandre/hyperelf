...
if __name__ == "__main__":
    ...
    for exp_folder, exp_name in \
        ...
        if df_concat is None:
            ...
            df_concat['avg entropy'] = report_csv['entropy']
            df_concat['std entropy'] = report_csv['entropy']**2
        else:
            df_concat['avg entropy'] += report_csv['entropy']
            df_concat['std entropy'] += report_csv['entropy']**2
        df_concat.insert(0, exp_name, report_csv['pred'])
    df_concat['avg entropy'] /= len(experiment_names)

    # Ref: https://en.wikipedia.org/wiki/Standard_deviation#Identities_and_mathematical_properties
    df_concat['std entropy'] = \
        (df_concat['std entropy']/len(experiment_names) - df_concat['avg entropy']**2)**(0.5)
    ...
    for i in range(num_classifiers+1):
        ...
        # Histogram on entropy per voting group
        entropy_groups.append(
            (
                f"{i}-{num_classifiers-i}",
                df_concat[df_concat["Agreements - 0"] == i]["avg entropy"].sum()/size_vote_group
            )
        )
        # Histogram on standard deviation per voting group
        stdent = df_concat[df_concat["Agreements - 0"] == i]["std entropy"].sum()
        stderr_groups.append(
            (
                f"{i}-{num_classifiers-i}",
                stdent/np.sqrt(size_vote_group)
            )
        )
    aux_vote = {}
    aux_entropy = {}
